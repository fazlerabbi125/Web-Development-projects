...
<form @submit.prevent="handleSubmit" >
  <h2 class="title"> <slot> Form <!-- fallback content --></slot> </h2>
    ...
    <p>
        ...
        <select name="route" id="route" class="form-select" v-model="delivery" required>
        <option disabled value="">Select Route</option>
        <option v-for="(option,i) in options" :key="option.route" :value="options[i]">
          {{option.route}}
        </option>
        </select>
    </p>
    <p>
        ...
        <select name="weight" id="weight" class="form-select" v-model="weight" required>
    ...
    <p>
        ...
        <select name="method" id="method" class="form-select" v-model="method" required>
...
const emit = defineEmits(['submitForm'])
    ...
    const title= ref('');
    const delivery= ref('');
    const options= ref([{price:50,route:'Inside Dhaka'},{price:150,route:'Outside Dhaka'}]);
    const weight= ref('');
    ...
    const handleSubmit= ()=>{
      if (title.value.length>=3){
          emit('submitForm',title.value,delivery.value.route,weight.value.name,cost.value,method.value,date.value.toLocaleString());
      }
      else {
