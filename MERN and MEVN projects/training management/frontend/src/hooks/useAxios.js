...
async function (error) { //renewing Token
  ...
  if (decodedUser && isExpired && status===403){
    const {data}= await axInstance.post("/refresh-token",{token:refreshToken});
...
const useAxios = (url,dataTimeout=1000) => {
    ...
    const fetchData = async ()=>{
      try {
        ...
        if (!response.data && !response.data.success) throw new Error(response.response.data.message);
