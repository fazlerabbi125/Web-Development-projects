...
class AuthController {
    ...
    async getProfile(req, res, next) {
        try {
            ...
            const user = await User.findById(id).select('-resetPasswordExpire -resetPasswordToken -emailVerificationToken').exec();
            if (!user) return res.status(HTTP_STATUS.BAD_REQUEST).send(failure('User not found'));
            return res.status(HTTP_STATUS.OK).send(success('User fetched successfully!', user));
        } catch (error) {
            next(error);
        }
    }
    ...
    async updateProfile(req, res, next) {
        try {
            ...
            if (!errors.isEmpty()) {
                if (req.file) {
                    await fs.promises.unlink(path.join(__dirname, '..', 'uploads','profiles',req.file.filename));
                }
            ...
            if (req.file||imgClear) {
                ...
                user.photo = req.file?process.env.BACKEND_URI+'/uploads/profiles/'+req.file.filename:'';
            }
            ...
            return res.status(HTTP_STATUS.OK).send(success('User is updated successfully!', resData));
    ...
    async verifyEmail(req, res,next) {
        try {
            ...
            const user = await User.findOne( {_id:id,emailVerificationExpire: { $gt: Date.now() }} ).exec();

            if (!user || user.emailVerificationToken !== token) {
                return res.status(HTTP_STATUS.FORBIDDEN).send(failure('Unsuccessful verification'));
            }
    ...
    async confirmEmailOnExpire(req, res, next) {
        try {
            ...
            const user = await User.findOne( {_id:id,emailVerificationExpire: { $lte: Date.now() }} ).exec();
            if (!user) {
                return res.status(HTTP_STATUS.FORBIDDEN).send(failure('Invalid email verification request'));
            }
            ...
            return res.status(HTTP_STATUS.OK).send(success('Verfication mail has been sent successfully'));
    ...
    async renewTokens(req, res, next) {
        ...
        try {
            ...
            const user= await User.findById(decodedData._id).select('_id name email role').exec();
            const userData={
                _id: user._id,
                name: user.name,
                email: user.email,
