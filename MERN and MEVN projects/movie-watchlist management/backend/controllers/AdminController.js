...
class AdminController {
    async addMovie(req, res, next) {
        try {
            ...
            const { title, genre, year, rating, description, country, language } =
                req.body;
            ...
            return res
                ...
                .send(success("Movie has added successfully", movie));
        } catch (error) {
            next(error);
        }
    }
    ...
    async editMovie(req, res, next) {
        try {
            ...
            if (updatedMovie) {
                ...
                if (req.file) {
                    if (updatedMovie.imgUrl.startsWith(process.env.BACKEND_URI)) {
                        const filepath = path.join(
                            __dirname,
                            "..",
                            updatedMovie.imgUrl.split(process.env.BACKEND_URI + "/")[1]
                        );
                        if (fs.existsSync(filepath)) await fs.promises.unlink(filepath);
                    }
                    updatedMovie.imgUrl = "/uploads/" + req.file.filename;
                } else if (!imgUrl) {
                    if (updatedMovie.imgUrl.startsWith(process.env.BACKEND_URI)) {
                        const filepath = path.join(
                            __dirname,
                ...
                return res
                    .status(HTTP_STATUS.OK)
                    .send(success("Movie details updated successfully", updatedMovie));
            }
            return res
                .status(HTTP_STATUS.BAD_REQUEST)
                .send(failure("Movie not found for update"));
        } catch (error) {
            next(error);
        }
    }
        ...
        try {
            ...
            if (movie.imgUrl.startsWith(process.env.BACKEND_URI)) {
                const filepath = path.join(
                    __dirname,
                    "..",
                    movie.imgUrl.split(process.env.BACKEND_URI + "/")[1]
                );
                if (fs.existsSync(filepath)) await fs.promises.unlink(filepath);
            }
            return res
                .status(HTTP_STATUS.OK)
                .send(success("Movie has been deleted successfully"));
        } catch (error) {
            next(error);
        }
    }
