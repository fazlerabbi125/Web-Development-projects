...
class MovieController {
    async getAll(req, res, next) {
        try {
            ...
            const { year, title, genre } = req.query;
            let movies, movieCount, itemsPerPage;

            if (year) {
                movieCount = await Movie.find({ year: parseInt(year) })
                    .count()
                    .exec();
                itemsPerPage = req.query.itemsPerPage
                    ? parseInt(req.query.itemsPerPage)
                    : movieCount;
                movies = await Movie.find({ year: parseInt(year) })
                    .skip(page * itemsPerPage)
                    .limit(itemsPerPage)
                    .exec();
            } else if (title) {
                const titleRegex = new RegExp(title, "i");
                movieCount = await Movie.find({ title: titleRegex }).count().exec();
                itemsPerPage = req.query.itemsPerPage
                    ? parseInt(req.query.itemsPerPage)
                    : movieCount;
                movies = await Movie.find({ title: titleRegex })
                    .skip(page * itemsPerPage)
                    .limit(itemsPerPage)
                    .exec();
            } else if (genre) {
                const genreRegex = new RegExp(genre, "i");
                movieCount = await Movie.find({ genre: genreRegex }).count().exec();
                itemsPerPage = req.query.itemsPerPage
                    ? parseInt(req.query.itemsPerPage)
                    : movieCount;
                movies = await Movie.find({ genre: genreRegex })
                    .skip(page * itemsPerPage)
                    .limit(itemsPerPage)
                    .exec();
            } else {
                movieCount = await Movie.find().count().exec();
                itemsPerPage = req.query.itemsPerPage
                    ? parseInt(req.query.itemsPerPage)
                    : movieCount;
            ...
            return res
                ...
                .send(success("All movies have been fetched successfully", { movies, movieCount }));
        } catch (error) {
            next(error);
        }
    }
        ...
        try {
            ...
            if (watchList) {
                ...
                if (watchList.movies.find((mov) => mov.movie == movID)) {
                    return res
                        .status(HTTP_STATUS.BAD_REQUEST)
                        .send(failure("Movie already exists in list"));
                } else await watchList.addToList(movID);
            } else {
                return res
                    .status(HTTP_STATUS.BAD_REQUEST)
                    .send(failure("This user has no watchList"));
            }
            return res
                .status(HTTP_STATUS.OK)
                .send(success("Movie has been added to list", watchList));
        } catch (error) {
            console.log(error);
            next(error);
        }
        ...
        try {
            ...
            if (movie) {
                ...
                return res
                    ...
                    .send(failure("Movie doesn't exist!!"));
            }
            return res
                .status(HTTP_STATUS.OK)
                .send(success("Movie has been removed from list"));
        } catch (error) {
            console.log(error);
            next(error);
        }
    ...
    async putChangeStatus(req, res, next) {
        try {
            ...
            return res
                .status(HTTP_STATUS.OK)
                .send(success("Movie status has been changed"));
        } catch (error) {
            console.log(error);
            next(error);
        }
    ...
    async getWatchList(req, res, next) {
        ...
        try {
            ...
            const userlist = await WatchList.findOne({ userId: req.user._id })
                .populate("userId", "name -_id")
                .populate("movies.movie")
                .exec();
            const itemsPerPage = req.query.itemsPerPage
                ? Number(req.query.itemsPerPage)
                : userlist.movies.length;
            ...
            return res.status(HTTP_STATUS.OK).send(
                success("Movies have been successfully fetched from userlist", {
                    userlist,
                    count,
                })
            );
