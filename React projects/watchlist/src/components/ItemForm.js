...
import {status,genres} from "../utils/constants"

const ItemForm = ({item,submitForm}) => {
    const navigate = useNavigate(); //hook for re-direct
    const dV={status:item.status||'',title:item.title||'',
    genre:(typeof item.genre === 'string'?item.genre.split('|'):item.genre)||[],year:item.year||null,
    rating:item.rating||null,description:item.description||'',country:item.country||'',language:item.language||''}
    const { register, handleSubmit, watch, formState:{errors} } = useForm({
    defaultValues:dV
    });
    const onSubmit = (data) => {
        const inputs=watch();
        inputs.genre=inputs.genre.reduce((str,value)=>{
    ...
    return ( 
        <>
            ...
            <div className="row justify-content-center ">
                ...
                {errors.status && <div className="text-center text-danger fw-bolder">{errors.status.message}</div>}    
            </div>
            <div className="mb-3">
            <div className="row justify-content-center ">
                <div className="col-auto">
