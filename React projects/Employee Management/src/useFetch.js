...
const useFetch = (url) => {
  ...
  const [isLoading, setIsLoading] = useState(true);//Used for conditional JSX
  ...
  useEffect(() => {
     ...
     setTimeout(()=>{ //Loads message for some time until data fetch is completed
        ...
        .then(data => {
          console.log(data);
          setData(data);
          setIsLoading(false);//Used for conditional JSX
          setError(null);
        })
        .catch((err)=>{
