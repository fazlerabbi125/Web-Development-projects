...
const EmployeeList: React.FC<EmployeeListProps> = ({ employeeList }) => {
  ...
  return (
    <>
      ...
      <h2
        style={{
          display: filteredList.length === 0 ? "block" : "none",
          color: "white",
          textAlign: "center",
        }}
      >
      ...
      {filteredList && (
        <div className={styles["emp-list"]}>
          {displayItems.map((employee) => (
            <Link
              to={`/employee/${employee.id}/details`}
              className={styles["emp-list__item"]}
              key={employee.id}
            >
              <img
                src={profileImg}
                alt="profile"
                className={styles["emp-list__item__photo"]}
              />
              <h2 className={styles["emp-list__item__heading"]}>
                {employee.name} - {employee.id}
              </h2>
              <p>
                <span className={styles["emp-list__item__property"]}>
                  Department:
                </span>{" "}
                {employee.dept}
              </p>
